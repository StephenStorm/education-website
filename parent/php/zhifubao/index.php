...
if(!isset($_SESSION['username'])){//用户未登录
	...
	if(empty($_POST['select'])){//没有选择
		header("location:../shopping_cart.php");
	}else{//选择了;保存到session中;计算总价
		...
		for($i=0;$i<count($_POST['select']);$i++){
			$result=$db->query('select fee*length as money from purchase natural join course where p_id="'.$username.'" and c_id="'.$_POST['select'][$i].'"');
...
：</dt>
                    <dd>
                        <input id="WIDtotal_amount" name="WIDtotal_amount" readonly="readonly" value="<?php echo $total;?>"/>
                    </dd>
                    <hr class="one_line">
                    <dt>商品描述：</dt>
                    <dd>
...
<script language="javascript">
	...
	//	document.getElementById("WIDtotal_amount").value = "5.01";
	}
	GetDateNow();
</script>
